...
import { getStaffNotices, markStaffNoticeRead } from '@/api/noticesApi'
...
const StaffNotices = () => {
  ...
  const fetchNotices = async () => {
    try {
      ...
      setNotices(res.data.notices || res.data.data || [])
    } catch (err) {
  ...
  const handleOpenNotice = async (notice) => {
    ...
    if (!notice.is_read) {
      try {
        await markStaffNoticeRead(notice.id, notice.source || 'unified')
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-32 rounded-2xl animate-pulse bg-slate-100" />
          ))}
        </div>
      ) : notices.length > 0 ? (
        <div className="grid gap-4">
          {notices.map((n) => (
            ...
            >
              {!n.is_read && (
                <div className="absolute top-4 right-4 w-2.5 h-2.5 rounded-full bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
              )}
        ...
        <div className="py-20 text-center space-y-4 rounded-2xl border-2 border-dashed border-slate-200">
