import { Clock, Calendar } from 'lucide-react'
import { getInitials } from '@/utils/helpers'
import Select from '@/components/ui/Select'

const TabTimeTable = () => {
  const days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']
  
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
        <h3 className="text-sm font-bold text-gray-900 flex items-center gap-2">
          <Clock size={16} className="text-indigo-600" /> Weekly Schedule
        </h3>
        <div className="flex items-center gap-2">
          <Select size="sm" options={[{ value: 'this', label: 'This Year' }]} defaultValue="this" containerClassName="w-32" />
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-5 divide-x divide-gray-100">
        {days.map(day => (
          <div key={day} className="space-y-4 p-4 min-w-[200px]">
            <h4 className="text-xs font-black uppercase text-gray-400 tracking-widest text-center border-b pb-3 mb-6">{day}</h4>
            <div className="space-y-4">
              <Slot 
                subject="Maths" 
                time="09:00 - 09:45 AM" 
                teacher="Jacquelin" 
                color="bg-rose-50 border-rose-100 text-rose-700 shadow-rose-100/50" 
              />
              <Slot 
                subject={day === 'Tuesday' ? 'Spanish' : 'English'} 
                time="09:45 - 10:30 AM" 
                teacher={day === 'Tuesday' ? 'Erickson' : 'Hellana'} 
                color={day === 'Tuesday' ? 'bg-sky-50 border-sky-100 text-sky-700 shadow-sky-100/50' : 'bg-orange-50 border-orange-100 text-orange-700 shadow-orange-100/50'} 
              />
              <Slot 
                subject="Computer" 
                time="10:45 - 11:30 AM" 
                teacher="Daniel" 
                color="bg-emerald-50 border-emerald-100 text-emerald-700 shadow-emerald-100/50" 
              />
              <Slot 
                subject={day === 'Thursday' ? 'Physics' : 'Maths'} 
                time="11:30 - 12:15 PM" 
                teacher={day === 'Thursday' ? 'Teresa' : 'Jacquelin'} 
                color={day === 'Thursday' ? 'bg-indigo-50 border-indigo-100 text-indigo-700 shadow-indigo-100/50' : 'bg-rose-50 border-rose-100 text-rose-700 shadow-rose-100/50'} 
              />
            </div>
          </div>
        ))}
      </div>
    </div>
  )
}

const Slot = ({ subject, time, teacher, color }) => (
  <div className={`p-3 rounded-xl border shadow-sm transition-transform hover:scale-[1.02] cursor-default ${color} space-y-3`}>
    <div className="space-y-1">
      <div className="flex items-center gap-1.5 opacity-70">
        <Clock size={12} strokeWidth={2.5} />
        <span className="text-[10px] font-black uppercase tracking-tighter">{time}</span>
      </div>
      <p className="text-xs font-black">Subject : {subject}</p>
    </div>
    <div className="flex items-center gap-2 pt-2 border-t border-current border-dashed border-opacity-20">
      <div className="h-6 w-6 rounded-lg overflow-hidden bg-white/50 border border-current border-opacity-20 flex items-center justify-center font-black text-[10px]">
        {getInitials(teacher)}
      </div>
      <span className="text-[10px] font-black tracking-tight">{teacher}</span>
    </div>
  </div>
)

export default TabTimeTable
